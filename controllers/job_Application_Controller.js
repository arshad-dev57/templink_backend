  ...
  // ==================== GET HIRED EMPLOYEES (TEAM) WITH PAGINATION ====================
  exports.getHiredEmployees = async (req, res) => {
    try {
      const employerId = req.user.id;
      const { 
        status,           // 'active', 'left', 'terminated' — optional filter
      ...
      // Employer find karo aur teamMembers ko populate karo
      const employer = await User.findById(employerId)
        .select('employerProfile.teamMembers role')
        .populate({
          path: 'employerProfile.teamMembers.employeeId',
          select: 'firstName lastName email employeeProfile' // employee ki zaroori fields
        });
      ...
      let allTeamMembers = employer.employerProfile?.teamMembers || [];

      // Agar status filter diya hai to apply karo
      if (status) {
        allTeamMembers = allTeamMembers.filter(m => m.status === status);
      }

      // Total count after filtering (pagination ke liye)
      const totalItems = allTeamMembers.length;

      // Pagination apply karo: slice method se specific items lo
      const paginatedTeamMembers = allTeamMembers.slice(skip, skip + limitNum);

      // Response format: employee details alag se dikhao
      const formatted = paginatedTeamMembers.map(member => ({
        _id: member._id,
        ...
        employeeDetails: member.employeeId // populated employee object
      }));

      // Summary counts (filtered data ke according, paginated nahi)
      const summary = {
        total: allTeamMembers.length,
        active: allTeamMembers.filter(m => m.status === 'active').length,
        left: allTeamMembers.filter(m => m.status === 'left').length,
        terminated: allTeamMembers.filter(m => m.status === 'terminated').length
      };
