...
exports.getMyJobPosts = async (req, res) => {
  try {
    ...
    const totalJobs = await JobPost.countDocuments({ postedBy: employerId });

    // ✅ Get jobs by this employer with pagination
  ...
  exports.createBulkJobPosts = async (req, res) => {
    try {
      if (!req.user?.id) {
        return res.status(401).json({ message: 'Unauthorized' });
      }
      ...
      const employerSnapshot = {
        ...
        about: ep.about || "",
        mission: ep.mission || "",
        cultureTags: ep.cultureTags || [],
        teamMembers: ep.teamMembers || [],
        isVerifiedEmployer: !!ep.isVerifiedEmployer,
        rating: ep.rating ?? 0,
        sizeLabel: ep.sizeLabel || "",
      };
      ...
      const createdJobs = await JobPost.insertMany(jobDocs);
